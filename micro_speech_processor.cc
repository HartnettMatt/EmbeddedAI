...
#include "models/matt_micro_speech_quantized_model_data.h"
...
namespace
{
  ...
  constexpr size_t kMicroSpeechArenaSize = 1024 * 256; // xtensa p6
  alignas(16) uint8_t g_micro_speech_arena[kMicroSpeechArenaSize];

  using MicroSpeechOpResolver = tflite::MicroMutableOpResolver<10>;
...
TfLiteStatus MicroSpeechModelInitialize()
{
  ...
  static const tflite::Model *model =
      tflite::GetModel(g_matt_micro_speech_quantized_model_data);
  if (model == nullptr)
  {
